...
BASE_SYSTEM_PROMPT = (
    ...
    "\n"
    "Each tag is a concrete sensory specific — a thing, a material, a "
    "light or color quality, an atmospheric condition, a posture, a "
    "moment. One concrete subject anchors the scene; the remaining tags "
    "are vivid supporting details that paint the rest of it. Be "
    "specific. Be evocative. Choose words a painter or cinematographer "
    "would choose.\n"
    "\n"
    "ANTI-LOOP RULE: do NOT default to a fixed palette. Pull from any "
    "era, any material world, any medium — biology, geology, fashion, "
    "ritual, industry, weather, architecture, food, music. If you used "
    "a word in the last 2 turns, pick a different one. Your vocabulary "
    "should feel like a different dictionary every turn.\n"
    "\n"
    ...
    "\n"
    "Each turn EVOLVES the dream — vary the subject, the lighting, the "
    "time of day, the angle, the mood. Different specific instances "
    "each turn. Do not repeat the same scene twice.\n"
    "\n"
    "Output ONLY the comma-separated tags. No preamble, no commentary, "
    "no parentheses, no word counts, no notes — just the tags."
)
...
def build_system_prompt(style_hint: str | None, keywords: str | None = None) -> str:
    ...
    if keywords:
        parts.append(
            f"\n\nACTIVE INFLUENCES: {keywords}. This is the AUTHORITY on "
            "what kind of dream this is. Follow it literally and faithfully:\n"
            "- If it describes a SCENE ('birds flying in the sky', 'an "
            "abandoned mall at night'), generate literal variations of that "
            "scene — different times of day, weather, angles, specific "
            "instances. The scene itself is the dream, not a starting "
            "point to twist away from.\n"
            "- If it's an AESTHETIC or vibe ('glitchcore', 'liminal space', "
            "'art nouveau'), let it dictate the mood, palette, and "
            "vocabulary of every supporting tag.\n"
            "- If it's a SUBJECT word ('birds', 'cathedrals'), make a "
            "specific instance the concrete subject — different one each "
            "turn — with supporting tags that suit that subject's natural "
            "world, not an unrelated one.\n"
            "Do NOT transform the influence into something darker, weirder, "
            "more decayed, or more chaotic than what it describes. If the "
            "influence wants a bright sky, give a bright sky.\n"
            "\n"
            "The 10-word hard limit still applies — pick the 3-5 most "
            "evocative details and stop. A short evocative prompt renders "
            "better than a long one in LCM at 4 steps."
        )
    return "".join(parts)
